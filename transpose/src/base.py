...
class Transpose:
    def __init__(
        ...
        if self.perform_authorized_request(
            Block,
            "https://api.transpose.io/v0/block/blocks-by-number?block_number_below=1",
            api_key,
        ):
    ...
    def bulk_request(
        self,
        endpoint_response: List,
        requests_per_second: int = None,
        results_to_fetch: int = 999999999999,
    ) -> List:
    ...
    def perform_authorized_request(
        self, model: type, endpoint: str, api_key: str = None
    ):
        if endpoint is None:
            return None
        ...
        print(
            "\n{}\n  {}\n".format(
                endpoint.replace("https://api.transpose.io", self.host).split("?")[0],
                "\n  ".join(endpoint.split("?")[1].split("&")),
            )
        ) if self.verbose else None
        request = requests.get(
            endpoint.replace("https://api.transpose.io", self.host),
