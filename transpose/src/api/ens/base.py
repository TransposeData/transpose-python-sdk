...
from ...util.models import ENSRecord, ENSTransfer, TransposeModel
...
class ENS:
    ...
    def records_by_owner(
        self, owner_address: str = None, limit: int = 10
    ) -> List[ENSRecord]:
        return self.super.perform_authorized_request(
            ENSRecord, _records_by_owner(owner_address=owner_address, limit=limit)
        )

    # Get ENS Records by Account
    # https://docs.transpose.io/reference/ens-records-by-account
    def records_by_account(self, resolved_address: str) -> List[ENSRecord]:
        return self.super.perform_authorized_request(
            ENSRecord, _records_by_account(resolved_address=resolved_address)
        )
    ...
    def records_by_name(
        self,
        ens_names: str or list = None,
    ) -> List[ENSRecord]:
        return self.super.perform_authorized_request(
            ENSRecord, _records_by_name(ens_names=ens_names)
        )

    # Get ENS Records by ENS Token ID
    # https://docs.transpose.io/reference/get_ens-records-by-token-id
    def records_by_token_id(
        self,
        token_ids: int or list = None,
    ) -> List[ENSRecord]:
        return self.super.perform_authorized_request(
            ENSRecord, _records_by_token_id(token_ids=token_ids)
        )
