...
from ...util.models import Account, BlockModel, Log, Transaction, TransposeModel
...
class Block:
    ...
    def blocks_by_number(
        ...
        return self.super.perform_authorized_request(
            BlockModel,
            _blocks_by_number(
                block_number_above=block_number_above,
                block_number_below=block_number_below,
                order=order,
                limit=limit,
            ),
        )
    ...
    def transactions_by_hash(
        self,
        transaction_hashes: str or list = None,
    ) -> List[Transaction]:
        return self.super.perform_authorized_request(
            Transaction, _transactions_by_hash(transaction_hashes=transaction_hashes)
        )

    # Get Transactions by Block
    # https://api.transpose.io/v0/block/transactions-by-block
    def transactions_by_block(
        self,
        block_number_above: int = 1,
        block_number_below: int = 1000000000,
        order: str = "asc",
    ...
    def logs_by_transaction(
        self, transaction_hash: str = None, limit: int = 10
    ) -> List[Log]:
        return self.super.perform_authorized_request(
            Log, _logs_by_transaction(transaction_hash=transaction_hash, limit=limit)
        )
